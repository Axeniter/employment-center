...
namespace EmploymentApp.Viewmodels
{
    ...
    public class ResponseWithApplicantViewModel : ObservableObject
    {
        public int Id { get; set; }
        public int VacancyId { get; set; }
        public string ApplicantId { get; set; }
    ...
    public partial class VacancyDetailEmployerViewModel : ObservableObject
    {
        ...
        [ObservableProperty]
        private ObservableCollection<ResponseWithApplicantViewModel> responses = new();
        ...
        public async Task LoadVacancyDetails()
        {
            ...
            try
            {
                ...
                var vacancy = await _apiClient.GetAsJsonAsync<VacancyDetailModel>(
                    $"/vacancies/{VacancyId}",
                    token
                );
        ...
        private async Task LoadResponses(string token)
        {
            try
            {
                var responseList = await _apiClient.GetAsJsonAsync<List<VacancyResponseModel>>(
                ...
                if (responseList != null && responseList.Count > 0)
                {
                    ...
                    {
                        var responseViewModel = new ResponseWithApplicantViewModel
                        {
                            Id = response.Id,
        ...
        private async Task LoadApplicantProfile(ResponseWithApplicantViewModel response, string token)
        {
            try
            {
                var profile = await _apiClient.GetAsJsonAsync<ApplicantProfileModel>(
